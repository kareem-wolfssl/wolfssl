...
static int TLSX_KeyShare_GenOqsKey(WOLFSSL *ssl, KeyShareEntry* kse)
{
    ...
    algName = OQS_KEM_alg_ntru_hps4096821;
    if (algName == NULL) {
...
// TODO: Add HAVE_KYBER/NTRU/etc?
    ...
    ret = TLSX_UseSupportedCurve(extensions, WOLFSSL_NTRU_HPS4096821,
                                 ssl->heap);
    if (ret != WOLFSSL_SUCCESS) return ret;
    ret = TLSX_UseSupportedCurve(extensions, WOLFSSL_NTRU_HRSS701, ssl->heap);
    if (ret != WOLFSSL_SUCCESS) return ret;
    ...
    if (ret != WOLFSSL_SUCCESS) return ret;
    ret = TLSX_UseSupportedCurve(extensions, WOLFSSL_KYBER90S512, ssl->heap);
    if (ret != WOLFSSL_SUCCESS) return ret;
    ret = TLSX_UseSupportedCurve(extensions, WOLFSSL_KYBER90S768, ssl->heap);
    if (ret != WOLFSSL_SUCCESS) return ret;
    ret = TLSX_UseSupportedCurve(extensions, WOLFSSL_KYBER90S1024, ssl->heap);
    if (ret != WOLFSSL_SUCCESS) return ret;
#endif /* HAVE_LIBOQS */
